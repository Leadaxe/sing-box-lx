...
| Связанные | [[SPECS/002-XHTTP_CLIENT_TRANSPORT]] · [[SPECS/008-AWG_JUNK_PARAM_VALIDATION]] · [[SPECS/009-WIRESOCK_MASQUERADE_PROFILES]] |
...
образцу [[SPECS/008-AWG_JUNK_PARAM_VALIDATION]], а не падение в рантайме.
...
6. Комбинация с REALITY + Vision + XHTTP ([[SPECS/002-XHTTP_CLIENT_TRANSPORT]]) несёт трафик.
...
- **Эталон для дифф-тестов.** Нужен локальный Xray-сервер и сверка байтов по
  сообщениям; без него handshake отлаживается вслепую. Решить до PLAN.
- **Лицензия.** Xray-core MPL-2.0, sing-box GPL-3.0. MPL §3.3 совместимость даёт, но
  портированные файлы должны нести исходный copyright и отметку происхождения — как
  сделано для WireSock (MIT) в [[SPECS/009-WIRESOCK_MASQUERADE_PROFILES]]. Проверить
  до мержа.
- **Приоритет.** Насколько распространены такие серверы у аудитории форка — от этого
  зависит, брать ли сейчас (ср. [[SPECS/024-RUNTIME_LOOP_GUARD]], отложенную до
  воспроизводимого кейса).
