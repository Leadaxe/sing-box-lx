...
**Дата:** 2026-06-17 · **Статус:** Open — код+тесты+DoD локально, **не закоммичено**,
ждёт ручного ревью · **База:** ветка `lx` (v1.13.13-lx)
...
- **Живой WARP/AWG-сервер не проверялся** (недоступен) — проверено только
  локально (`check` + структурный парсинг + реальный `newObfChain`). Это decoy
  перед handshake, корректность приёма движком и структуры подтверждена; полевая
  проверка против реального DPI — отдельный шаг.
