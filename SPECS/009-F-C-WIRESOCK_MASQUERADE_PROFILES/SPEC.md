...
> **Слабое звено H1 (держать как непроверенное условие).** `:2408` — это **WireGuard**-порт WARP.
> Что Cloudflare-edge отвечает QUIC/HTTP3 **именно на этом UDP-порту** (а не на штатном `:443`), в
> этом репозитории **не снято пакетным захватом**. «Edge говорит QUIC по всему флоту» НЕ равно
> «этот порт отвечает на QUIC Initial». Поэтому формулировку «Cloudflare отвечает QUIC на `:2408`»
> нельзя заявлять как факт — только как условие, которое разрешает тест T3.
...
(модель §3), responder не чинит ничего: проблема не в отсутствии ответа, а в том, что raw
DNS/STUN/SIP к дата-центровому IP аномальны по назначению — починит только **протокол-уместное
назначение** (DNS на `:53` и т.д.), не co-located responder на том же WG-порту. То есть вывод
«не-QUIC нужен self-hosted responder» **корректен только при активной модели**; не выдаём его за
установленный факт. Именно поэтому код заполняет только `i1` (QUIC для WARP), а `i2..i5` оставлены
свободными под self-hosted/мульти-decoy — это реальный задел, не гипотетический.

Прозу про репутацию назначения **не дублируем**: она уже есть в `transport/wireguard/masque_awg.go`
(DNS-генератор) и в §3 — этот раздел их обобщает в probe-модель. Серверная сторона / probe-response
— вне скоупа (§10). Источник device-фактов — LxBox-задача 146; статья habr 1047080 описывает
двустороннюю модель WireSock архитектурно (без байт-спек и без измерений) — подтверждает
...
Этот раздел — **гипотеза дизайна, НЕ принятое решение и НЕ device-факт.** Идея: разложить
QUIC-приманку на несколько пакетов `i1..i5` («развивающаяся» QUIC-сессия), как описывает
WireSock-статья (Initial → Handshake → Protected). Документируем, потому что слоты `i2..i5` уже
существуют в движке (`device.go` `ipackets [5]*obfChain`, каждый парсится самостоятельно в
`uapi.go`, каждый шлётся своим UDP-датаграмом в `send.go`), и потому что задел из §8 (H3) живёт
именно здесь.

> **Серьёзная оговорка (history-grounded).** Конкретный байт, который напрашивается для `i2` —
> QUIC **short-header (1-RTT)** «continuation» — это **тот самый конструкт, который коммит
> `64ce4a47` УДАЛИЛ как device-blocked.** Шапка `quic_initial_awg.go` прямо предупреждает:
> ip=quic-маскировка эмитила QUIC 1-RTT short header, и он был **эмпирически заблокирован** реальным
> LTE-DPI (device-A/B), все short-header-варианты давали Timeout. Поэтому short-header `i2` — **не
> новая идея, а кандидат на регресс уже сожжённого device-цикла.** Если идём в multi-packet,
> **второй фрагментированный Initial** (или Initial→Handshake-образный пакет) защитимее, чем
> воскрешение удалённого short header.

### 9.1 Почему это не сломает WARP (ортогональность — verified)

Разобрано построчно по `submodules/wireguard-go/device/send.go` `SendHandshakeInitiation` и
**подтверждено** при ревью: `i2` (как и любой decoy `i2..i5`) **не может** задержать,
переупорядочить, испортить или изменить ни одного валидируемого байта реального `MessageInitiation`.
...
### 9.2 Что под вопросом (НЕ verified) и риски

Вторая половина — «улучшает ли это правдоподобность» — **не обоснована и по device-данным скорее
контрпродуктивна.**

1. **Short-header `i2` уже device-blocked** (см. оговорку). Единственное, что могло бы спасти —
   проверить, читается ли 1-RTT-пакет, которому **предшествует прошедший Initial**, иначе, чем
   заблокированный standalone short header. Доказательств ноль; считать гейтом, не данностью.
2. **DCID-reuse — скорее fingerprint, чем «плюс».** Настоящий QUIC-клиент во втором пакете
   использует **server-chosen** CID после первого флайта сервера и не шлёт 1-RTT short-header со
   своим исходным DCID без промежуточного ответа сервера. «Клиент шлёт 1-RTT до завершения
   серверного handshake, переиспользуя свой DCID» — **невозможное QUIC-состояние**, то есть более
   чистая аномалия, чем одинокий фрагментированный Initial. Не подаём DCID-reuse как «good».
3. **Retry-амплификация.** `SendHandshakeInitiation` гоняет весь decoy-цикл **на каждом retry**. На
   лоссовом LTE, где теряется именно подлинный пакет, дизайн **увеличивает** head-of-line-байты
   (`i1`~1250б + `i2`~700–1100б + `jc`) ровно когда линк хуже всего. «decoy+jc — один бюджет»
   должно быть **жёстким лимитом**: суммарно ≤3–4 ведущих датаграма; `ip=quic` уже тратит часть —
   не задирать `jc` сверху.
4. **Flood/anti-amplification.** 2 decoy + малый `jc` — ниже правдоподобного порога, та же форма
   всплеска, что `jc` уже даёт; не превышать ~3–4 ведущих датаграма.
5. **MTU.** `i2` ≤1200б и **не больше `i1`** → нет IP-фрагментации на LTE.
6. **Регресс проверенного `i1`.** Любой refactor (напр. подъём генерации DCID в `masqueI1` ради
   общего DCID для `i1` и `i2`) обязан оставить байты `i1` структурно идентичными и сопровождаться
   тестом, что `i1` по-прежнему реассемблируется обратным парсером с внешне-поданным DCID.
7. **Нет серверного responder.** `i2` помогает (если вообще) только против **пассивной**
   flow-классификации, НЕ против активной пробы (§8) — ответить на QUIC Version Negotiation мы не
   можем (WARP не наш).

### 9.3 Что device-тестировать ДО отгрузки

Byte-level DPI-спеки нет; всё эмпирически. Учитывая, что short header уже **давал Timeout**,
реалистичное ожидание — регресс; планка не «не дестабилизирует», а **`i2` должен быть не хуже /
лучше `i1`-only**.

1. **A/B `i1+i2` vs `i1`-only:** держит ли туннель на ~340 мс? Контроль — `i1`-only; если `i2`
   регрессит — **шлём `i1`-only**.
2. **Меняет ли вердикт DPI то, что 1-RTT-пакету предшествует прошедший Initial** (vs заблокированный
   standalone short header)? Единственное, что может спасти идею; доказательств нет.
3. **Shared-DCID vs independent-DCID `i2`** — shared-DCID считать **подозрительно-худшей** ветвью
   (п.2 рисков), не нейтральной.
4. **Retry на лоссовом аплинке:** `i1+i2+jc` при типовом `jc` — приходит ли подлинный
   `MessageInitiation` в пределах `RekeyTimeout` на **первой** попытке.
5. **Sweep размера `i2`** {~700, ~900, ~1100}б (cap ≤1200б, ≤`i1`).
6. **`i3..i5` (отложено):** третий пакет — только после доказанной 2-пакетной базы и внутри лимита.
7. **Проводка `i2` (пока НЕ построена):** `device_awg.go` пишет `i2` из `o.I2` **без** masque-пути
   и **без** guard'а взаимоисключения. Auto-fill `i2` из sugar-слоя требует нового кода + guard'а,
   зеркального `i1`-конфликту. Как есть — у маппинга «`i2` = …» нет хука в коде.
...
- Многопакетная QUIC-последовательность (§9) — спроектирована, но НЕ реализована: ждёт device-A/B.
