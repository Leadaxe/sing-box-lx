...
| **036** | DNS_GROUP_TRACE | **C** | 🔬 Наблюдаемость группы для LxBox: трасса проб в `SubscribeDNSQueries` (путь групп, хронология с исходами/rtt, признак гонщика; прото аддитивно), state-RPC `GetDNSGroups` (§3.6, размороженный §4 SPEC 035 — запрос LxBox есть), лог решений. Write-once effective чинит атрибуцию вложенных групп. [SPEC.md](TASKS/036-DNS_GROUP_TRACE/SPEC.md) |
