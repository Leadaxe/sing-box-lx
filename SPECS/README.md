...
| **033** | DNS_GROUP_SERVER | **O** | 🧭 DNS-группа: тип `group`, режим `failover`, `down_time`, валидации (циклы, состав, `servers`). Фича [DNS_GROUP](FEATURES/DNS_GROUP/FEATURE.md). [SPEC.md](TASKS/033-DNS_GROUP_SERVER/SPEC.md) |
| **034** | DNS_GROUP_RACE | **O** | 🏁 Режим `race`: ленивая гонка на реальном запросе, победитель + рейтинг, без таймеров. [SPEC.md](TASKS/034-DNS_GROUP_RACE/SPEC.md) |
| **035** | DNS_GROUP_OBSERVABILITY | **O** | 👁 Фактический ответивший участник группы в `SubscribeDNSQueries` (proto не меняется). [SPEC.md](TASKS/035-DNS_GROUP_OBSERVABILITY/SPEC.md) |
