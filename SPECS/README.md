...
| **035** | DNS_GROUP_OBSERVABILITY | **C** | 👁 ✅ **Код+тесты+DoD (объединена с бывшей 036 по §3.2 — одна зона).** Наблюдаемость группы целиком: фактический участник в `SubscribeDNSQueries` (write-once effective — лист при вложенности), трасса проб (путь групп изнутри наружу, хронология с исходами/rtt, признак гонщика; прото аддитивно, поля 13–15), state-RPC `GetDNSGroups` (§3.6, аналог GetPool), лог решений (down/гонка/смена победителя debug, исчерпание warn). Живой box.New: опоздавший ответ гонки не в снимке трассы. Контракт LxBox — в IMPLEMENTATION_REPORT. [SPEC.md](TASKS/035-DNS_GROUP_OBSERVABILITY/SPEC.md) |
