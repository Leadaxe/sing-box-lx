...
| **100** | LXD_ROOT_OWNED_BINARY | **O** | Решение владельца 2026-09-24: `lxd --service=install` писал в plist LaunchDaemon путь, из которого запущен (бинарь в бандле лаунчера, владелец — пользователь), и root исполнял user-writable файл — LPE. Служба исполняет root-owned копию `/Library/PrivilegedHelperTools/com.leadaxe.sing-box-lxd/sing-box` (temp+fsync+chown+sha+rename, пропуск при совпадении sha) с сайдкаром `install.json`; `--exec-dir` с проверкой цепочки от `/`; самопроверка демона под launchd; `--service=status` с кодами выхода; uninstall только по сайдкару; `/admin/info` отдаёт `executable`/`executable_sha256`; `GOOS=darwin go vet` в CI. [SPEC.md](TASKS/100-LXD_ROOT_OWNED_BINARY/SPEC.md) |
