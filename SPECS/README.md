...
| **103** | LXD_WINDOWS_SERVICE | **W** | Решение владельца 2026-09-24: `lxd` как служба Windows (SCM `sing-box-lxd`, LocalSystem, автозапуск), перенос модели 100: служба исполняет защищённую копию `<ProgramFiles>\sing-box-lxd\` (exe + `libcronet.dll`, сайдкар с `files[]`), инвариант по владельцу и DACL, захват `<ProgramData>\sing-box-lxd`, `--service=install/copy/uninstall/status` с кодами 100, `svc.Run`, лог с ротацией, `--invite-out`, `SetDefaultDllDirectories`. Пара SPEC 141 лаунчера; цель v1.14.2-lx.2; в реализации. [SPEC.md](TASKS/103-LXD_WINDOWS_SERVICE/SPEC.md) |
