...
| **094** | XHTTP_LOCAL_CLOSE_NOT_FAILURE | **I** | LxBox #148: под urltest в selector'е с `interrupt_exist_connections: true` XHTTP-соединения вытеснялись как `failing`, а лог был полон `connection download closed: http2: response body closed` на ERROR. Два наших дефекта на границе xhttp-conn: брейкер xmux считал сбоем `context.Canceled` от нашего же `Close()`; закрытое нами тело отдавало релею неизвестный sing sentinel. Теперь `Canceled` нейтрален (DeadlineExceeded и удалённые ошибки остаются сбоями), под гейтом `localClosed` Read отдаёт `net.ErrClosed` или `os.ErrDeadlineExceeded`. Стражи red/green, `option/` не тронут. Живой стенд и подтверждение репортёра впереди. [SPEC.md](TASKS/094-XHTTP_LOCAL_CLOSE_NOT_FAILURE/SPEC.md) |
