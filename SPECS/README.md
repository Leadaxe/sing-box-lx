...
| **033** | DNS_GROUP_SERVER | **O** | 🧭 **v2 в работе: TTL-модель.** Режимы `stable`/`fastest`/`parallel` как грани одного механизма (034 слита сюда по §3.2, номер свободен); две TTL-записи (ошибка/победа, успех↔ошибка симметрично стирают), веер по чистым с гарантированным бюджетом, анти-шторм «чистых нет → одна попытка», single-flight выборов. v1 (failover/race/down_time, отгружена в rc.1) — в HISTORY.md; конфиг rc.1 сознательно ломается (потребителей нет). [SPEC.md](TASKS/033-DNS_GROUP_SERVER/SPEC.md) |
| **035** | DNS_GROUP_OBSERVABILITY | **O** | 👁 **v3 в работе: стык с TTL-моделью.** `racer`→`fanned` + новый `survival` (видимость деградации), state-RPC v3 (`clean`/`liveErrors`/`lastErrorAgeMs`/`liveWins`/`current` вместо winner/ranking/down-полей — замена, v2-прото не был в релизе). Механизм трассы (write-once effective, path изнутри наружу, снимок под мьютексом) без изменений. [SPEC.md](TASKS/035-DNS_GROUP_OBSERVABILITY/SPEC.md) |
