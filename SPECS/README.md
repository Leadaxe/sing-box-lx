...
| **007** | AWG_OVER_WIREGUARD_DETOUR_GUARD | **C** | ✅ **Код+тесты+DoD** (field-verify по запросу). Bug в 003: AWG-нода с `detour` на WireGuard-туннель (плоский WG или AWG) вешает ядро на Android (AWG внутри WG). Guard по образцу `empty direct detour` — ленивая ошибка в `DetourDialer.init()`, **вариант B** (узел не встаёт, ядро живо). detour на VLESS и WG→AWG — разрешены. Чинит [#2](https://github.com/Leadaxe/sing-box-lx/issues/2) |
