...
| **031** | AWG_PARITY_AUDIT_ADVANCED_SECURITY | **N** | ✅ **Сверка паритета AWG: реализовано 16 из 16 параметров обфускации.** Ответ на «все ли фишки AWG мы понимаем/реализовали». У AmneziaWG **нет нормативной спеки** (ни RFC, ни whitepaper) — протокол задан реализациями, поэтому сверка сделана по `amneziawg-tools` (`src/config.c` — что принимает официальный инструмент в `.conf`) с кросс-проверкой netlink-контракта ядерного модуля. Совпадение полное: `Jc/Jmin/Jmax`, `S1–S4`, `H1–H4`, `I1–I5`, CPS-мини-язык 1:1 (8 тегов `b,d,ds,dz,r,rc,rd,t`). К серверу на официальном AWG клиент подключается. **Отозвано:** `J1–J3`/`Itime` (первая редакция этой спеки) — таких параметров в AWG нет ни в одной из трёх реализаций; «advanced»-ключи ветки `master` `amneziawg-go` (`header_protection_key`, `content_padding_addition`, 5 таймингов) — неreleased разработка: tools их не шлёт, модуль не знает ⇒ `master` одной реализации ≠ протокол. Единственный зазор: **`AdvancedSecurity`** (boolean) — по коду гейтит валидацию junk и учёт состояния пира, на send-пути отсутствует ⇒ блокером не выглядит (гипотеза, живьём не проверена); ~50–100 LOC, если подтвердится. [SPEC.md](TASKS/031-AWG_PARITY_AUDIT_ADVANCED_SECURITY/SPEC.md) |
