...
| **041** | WG_HANDSHAKE_GIVEUP_REBIND | **C** | 🩹 **WG/AWG-узлы после сна навсегда в ERR, лечил только реконнект (dewch #1298+).** Дампы: `StdNetBind`-сокеты живы 33 мин, порт не менялся — per-flow-состояние пути (NAT/DPI 5-tuple) умерло, а `wireguard-go` после give-up (90 с ретраев) вечно ретраит в мёртвый сокет. Фикс — пассивный self-heal в форке submodule: событие give-up один раз переоткрывает bind (свежий ephemeral-порт, если `listen_port` не пинован; дебаунс 90 с; на down/closed девайсе — no-op, совместимо со сном 020) и сразу кикает новое рукопожатие; masquerade `i1` уходит первой инициацией нового 5-tuple. Оба bind-пути (StdNetBind + ClientBind/detour) одним механизмом. Red/green e2e (blackhole первого сокета: база 20 с таймаут → фикс PASS) + юниты fresh/pinned/дебаунс/disabled. Остаток: field на стенде жалобы. [SPEC.md](TASKS/041-WG_HANDSHAKE_GIVEUP_REBIND/SPEC.md) |
