...
| **100** | LXD_ROOT_OWNED_BINARY | **I** | Решение владельца 2026-09-24: `lxd --service=install` писал в plist LaunchDaemon путь, из которого запущен (бинарь в бандле лаунчера, владелец — пользователь), и root исполнял user-writable файл — LPE. Служба исполняет root-owned копию `/Library/PrivilegedHelperTools/com.leadaxe.sing-box-lxd` — плоский файл по соглашению Apple (temp+fsync+chown+sha+rename, пропуск при совпадении sha) с сайдкаром `com.leadaxe.sing-box-lxd.install.json` (`plist_path`, `label`); каталог старой раскладки на месте копии — отказ с подсказкой `sudo rm -rf`; `--exec-dir` с проверкой цепочки от `/`; `--service=copy` — копия без службы для classic TUN лаунчера; `--service=status` с кодами 0/2/3/4; uninstall только по сайдкару; самопроверка `lxd`/`run` под root (отказ только в launchd-службе); `/admin/info` отдаёт `executable`/`executable_sha256`; `GOOS=darwin go vet` в CI. Табличный тест переходов состояний; ручная проверка под sudo — за владельцем. [SPEC.md](TASKS/100-LXD_ROOT_OWNED_BINARY/SPEC.md) |
