...
| **041** | WG_HANDSHAKE_GIVEUP_REBIND | **O** | 🩹 **WG/AWG-узлы после сна навсегда в ERR, лечил только реконнект (dewch #1298+).** Дампы: `StdNetBind`-сокеты живы 33 мин, порт не менялся — per-flow-состояние пути (NAT/DPI 5-tuple) умерло, а `wireguard-go` после give-up (90 с ретраев) вечно ретраит в мёртвый сокет. v1 (rc.4, ✅ synthetic + uphold): пассивный self-heal в форке submodule — событие give-up один раз переоткрывает bind (свежий ephemeral-порт, если `listen_port` не пинован; дебаунс 90 с; на down/closed девайсе — no-op, совместимо со сном 020) и сразу кикает новое рукопожатие; оба bind-пути одним механизмом. **v2 (open, полевой остаток по дампу 01.08):** v1 лечит, но лишь на ~90-й секунде — юзер меряет пинг в первые полминуты после пробуждения и видит ERR. Добавляются два триггера того же rebind: досрочный (≥3 неотвеченных инициации при доказуемо мёртвой сессии — нет keypair / handshake старше 180 с → ~15 с вместо 90) и событийный нудж libbox `RebindStaleEndpoints()` (LxBox дёргает на `USER_PRESENT`; здоровые/спящие узлы — no-op). Дебаунс общий, конфиг не расширяется. См. HISTORY.md. [SPEC.md](TASKS/041-WG_HANDSHAKE_GIVEUP_REBIND/SPEC.md) |
