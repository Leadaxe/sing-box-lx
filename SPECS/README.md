...
| **004** | BUILD_CI_RELEASE | **C** | ✅ `Makefile.lx`/libbox-теги, дешёвый CI (lint+build-check на push; cross×6+AAR на dispatch), `lx-release.yml` (**релиз v1.13.13-lx.3 опубликован** — 6 desktop + 2 AAR), поставка `libcronet.dll` в windows-архивах (naive/purego; darwin — dylib не существует, naive там нефункционален), `lx-rebase.yml` (авто-ребейз → PR/issue, демо зелёное) |
