...
- **`Now()` cold-start (least_test, rc.12).** До первого URL-теста история пуста,
  ...
  Заметь: когда процесс **не выгружался** из памяти (быстрый рестарт конфига на
  Android), история и `selectedOutbound*` переживают рестарт (in-memory
  `HistoryStorage`, без диска), поэтому `Now()` показывает прошлый выбор сразу, без
  прогрева — это и есть наблюдаемая «липкость выбора после перезапуска», by design
  апстрима.

  **Рассмотренная альтернатива (вариант B, отклонён).** Можно было записать
  fallback-узел прямо в `selectedOutboundTCP/UDP` при первом дайле/`Now()` (кэш
  заглушки), вместо доопроса `Select()` на чтение. Плюс B: `Now()` и `DialContext`
  читали бы **одно поле**, поэтому они не могут разойтись — гонки нет by
  construction. Минус B: это сильнее вмешивается в апстрим-логику — в поле выбора
  кладётся неизмеренная заглушка (`exists=false`), и первая реальная смена выбора
  даёт `+1 Interrupt()` (`performUpdateCheck` видит `selectedOutbound* != nil` →
  `updated=true`). Этот разрыв, по сути, **не отличается** от разрыва при любой
  последующей смене выбора по latency (апстрим и так рвёт соединения при каждом
  перевыборе), так что сам по себе он не аргумент против. **Выбран вариант A** —
  чисто из минимального вмешательства в апстрим (`Now()` остаётся читателем, поле
  `selectedOutbound*` заполняет только `performUpdateCheck` после реального теста,
  как в оригинале). Цена A — **ничтожный** микрозазор: `Now()` и дайл считают
  значение двумя **раздельными** вызовами `Select()`, и если результат теста
  прилетел ровно между ними на первых секундах, они могут показать разные узлы на
  одно мгновение. Окно — миллисекунды на холодном старте, разойтись могут только в
  этот момент; B убрал бы и его, но ценой вмешательства в апстрим, которое мы
  сознательно не хотим тащить. Если масштаб фичи вырастет (свои метрики выбора,
  отказ от апстримного `selectedOutbound*`) — переход на B становится оправданным.
