...
| [029](../../TASKS/029-ENDPOINT_DETOUR_START_ORDER/SPEC.md) | Endpoint с `detour` мёртв, если провайдер объявлен позже | `protocol/wireguard/endpoint.go` | Апстрим унесёт резолв detour под топосорт-барьер | держим |
...
**029 — порядок старта.** Резолв detour утекал в конструктор `NewEndpoint`
(egress-anchor-каст шёл по `Upstream()`, а `DetourDialer.Upstream()` жадно
резолвит) — то есть в фазу Create, до сборки графа. Провайдер, объявленный
в конфиге позже, ещё не был в реестре, и `sync.Once` навсегда кэшировал
`outbound detour not found`. Ядро упорядочивает старт топосортом, но резолв
уходил из-под барьера. Фикс: каст обёрнут в `if Detour==""`, а `InitializeDetour`
перенесён в `Start(StartStateStart)` — за барьер, с fail-fast.
