...
- **P11. Остановка во время запуска не роняет процесс.** Демон намеренно
  ...
  задокументированы в SPEC 070). Мутация: вызов транспортного `Start` вне
...
- **P12. Один мёртвый detour-узел не замораживает сетевую машинерию процесса.**
  ...
  очередью за ним (Send, Close, rebind). Реакция эндпоинта на pause-события
  (`Down`/`Up`) исполняется вне замка pause-менеджера — подписчик, упёршийся
  в мьютексы устройства, больше не останавливает доставку
  pause/wake/смены сети всему процессу; всплеск событий сходится к последнему
  (latest-wins), закрытие инвалидирует очередь. Свидетель: red/green юнит
  «дозвон в чёрную дыру возвращается в пределах бюджета и отпускает
  `connAccess`» (red = вечное зависание); юниты механики диспатча
  (не блокирует, latest-wins, инвалидация штампом закрытия); полевой дамп
  2026-08-12 как эталон симптома. Мутация: dial под `connAccess` без
  дедлайна; возврат синхронного `Down`/`Up` в pause-колбэк; отмена
  таймаут-контекста при возврате из connect (убивает живое соединение в окне
  сторожа 050).
...
| [070](../../TASKS/070-WG_START_CLOSE_RACE_CRASH/SPEC.md) | Стоп во время долгого старта (полевой профиль: 1755 outbounds + 18 WG-эндпоинтов) роняет весь процесс nil-паникой: демон отпускает `serviceAccess` на время `instance.Start()`, `CloseService` проходит гейт `STARTING` и ведёт `Box.Close` конкурентно с идущим `Box.Start`; `closeTunDevice` (SPEC 020) обнуляет tun-device под ногами транспортного `Start` → SIGSEGV на `SetDevice`. Вторая мина на том же пути — неатомарная идемпотентность `Box.Close` (double-close `done`) | `protocol/wireguard/endpoint.go` (`Start` под `resumeMu` + гейт `closing`), `box.go` (CAS в `Close`) — маркер `// lx: SPEC 070` | Апстрим сериализует жизненный цикл сам: `StartedService` перестанет закрывать недостартовавший инстанс либо `Box`/менеджеры сделают Start/Close взаимоисключающими; следить за расстановкой локов вокруг `instance.Start()` в `daemon/started_service.go` и за `Start`/`Close` в `protocol/wireguard/endpoint.go` на каждом мерже | держим — юнит red/green + смоук зелёные; остаток: полевой прогон на профиле жалобы; в окне живут ещё 3 апстрим-гонки без крашей (см. SPEC 070) |
| [071](../../TASKS/071-WG_BIND_DIAL_PAUSE_DEADLOCK/SPEC.md) | WG-эндпоинт через `detour` на полумёртвый узел (VLESS+XHTTP: TCP открылся, стрим не поднялся) замораживает сетевую машинерию всего процесса: dial в `ClientBind.connect()` без дедлайна виснет в непрочитанном upload-пайпе (полевой дамп: 54 мин), держит `connAccess` → голодают Send/Close/rebind; pause-менеджер `sing` зовёт колбэки под своим замком, наш `onPauseUpdated → Down()` упирается в те же мьютексы → доставка pause/wake/смены сети встаёт всему процессу; лечит только force-stop. Наши SPEC 020/041 умножают заходы в кольцо | `transport/wireguard/client_bind.go` (дедлайн `C.TCPTimeout` на dial, отмена по смерти wireConn — в связке со сторожем SPEC 050), `transport/wireguard/endpoint.go` (async latest-wins диспатч pause-событий под `pauseOpAccess`) — маркер `// lx: SPEC 071` | Fix 1: апстрим сам ограничит dial в `connect()` (следить за `client_bind.go` на мержах). Fix 2: `sing` перестанет звать pause-колбэки под `d.access` (следить за `service/pause/default.go` при бампах sing); диспатч тогда безвреден, но держать незачем | держим — юнит red/green (red = вечное зависание connect), сюиты `-race` зелёные; остаток: полевой прогон на профиле дампа |
