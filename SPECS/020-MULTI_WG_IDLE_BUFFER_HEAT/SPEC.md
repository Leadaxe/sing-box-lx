...
## 14. ОТВЕРГНУТО: эксперимент «GRO off + batch 8» как альтернатива Down/Up

Возник вопрос: а нужен ли вообще Down/Up, если глобально урезать recv-batch (128→8) —
тогда `bufsArrs` малы у ВСЕХ нод всегда, без suspend/wake, без reachability-walk, без
handshake на пробуждении. Изначально это отвергалось как «ломает GRO» (RESEARCH.md
рычаг 1). Решили всё-таки замерить. **Итог: идея отвергнута — по трём независимым
причинам.** Артефакты и сырые данные: [ANDROID_RESEARCH/nogro-experiment/](ANDROID_RESEARCH/nogro-experiment/)
(`RESULT-2026-07-01.md` + heap-снимки); код — на ветках `lx-1.14-nogro-exp` /
`lx-1.14-nogro-hard` (сабмодуль `lx-awg2-v003-nogro-*`), в prod НЕ вливается.

**Причина 1 — не тот держатель (главное, меняет постановку).** On-device heap показал:
основной RAM-холдер на android — это `device.pool.messageBuffers` (`pools.go`,
`PreallocatedBuffersPerPool = 4096 × ~64 КБ ≈ 100 МБ`, `PopulatePools.func3`, 82–86%
кучи), и он **от `BatchSize()` НЕ зависит вообще**. batch влияет только на `bufsArrs`
recv-воркеров, которые в том прогоне держали всего **~14 МБ**. Т.е. даже при успешном
batch 128→8 основные ~100 МБ никуда бы не делись. Настоящий рычаг RAM на android, если
он понадобится — **`PreallocatedBuffersPerPool`**, а НЕ GRO/batch. (Down/Up при этом
всё равно освобождает ВСЮ recv-инфраструктуру спящей ноды, потому и работает — §12.)

**Причина 2 — переключатель не доставляется на android.** env-канал `LX_WG_NO_GRO`
через `wrap.<pkg>` prop: переменная железно видна в `/proc/<pid>/environ`, но
`os.Getenv` в Go-рантайме libbox её НЕ видит (Go снимает `environ` при инициализации
`.so`, до/мимо prop-инъекции). См. [[android-goos-linux-file-suffix]]. Пришлось
хардкодить `lxNoGRO()=true` в бинаре.

**Причина 3 — хрупкая связка, паника.** Хардкод batch=8 крашил туннель при старте
(**SIGABRT**): `device.BatchSize() = max(bind, tun)` — Linux-TUN через vnet-hdr offload
всё равно отдавал `IdealBatchSize=128`, `max()` возвращал 128, а урезанный `msgsPool`
был 8 → `Send`: `(*msgs)[:len(bufs)]` = `[:128]` при cap 8 → out-of-range. Чтобы
починить, пришлось гейтить ЕЩЁ и TUN-offload (vnetHdr off, batchSize=1) — т.е.
согласованно трогать три разных слоя сабмодуля (bind + msgsPool + tun), и это ровно та
ловушка `max(bind,tun)`, о которой предупреждает [[wg-bindupdate-keys-safe]].
Фикс написан (ветка `-hard`), но НЕ проверен на устройстве — эксперимент свернули на
этом шаге.

**Вывод.** Путь «GRO off + batch» — сложный (три слоя сабмодуля, хрупкий `max()`,
паника), недоставляемый штатно (env не долетает), и **бьёт не в тот держатель** (batch
≠ главные 100 МБ). Down/Up (rc.19) остаётся единственным жизнеспособным механизмом: он
измеренно освобождает recv-инфраструктуру и не жертвует GRO активной ноды. Throughput
без GRO так и не замерен (переключатель не активировался) — но это уже неважно, т.к.
причина 1 снимает саму мотивацию.
