...
| Статус | **C (closed) — НЕ удалось воспроизвести на lx.14.** На той же ноде/сети, где ловился baseline, на ядре `1.13.13-lx.14` (с фиксом §010) баг download-сталла больше не воспроизводится — ни в обычном режиме, ни под зондом `LX_CONN_TRACE=0` (код = релиз). Вероятная причина: фикс **§010 GRO split-brain** (вошёл в lx.14) закрыл то, что документировалось здесь. См. раздел «Закрытие 22.06» ниже. |
| Зона | ядро `sing-box-lx` — `route/conn.go` (`connectionCopy`, download-направление `remoteConn→conn`); фактический фикс — submodule `wireguard-go` (GRO, §010) |
| Связь | **Вероятный дубликат / проявление** [010-WG_ENDPOINT_GRO_SPLIT_BRAIN](../010-B-C-WG_ENDPOINT_GRO_SPLIT_BRAIN/SPEC.md) — ИЗНАЧАЛЬНО помечалось «НЕ дубликат (виснет и на VLESS)», но это наблюдение не подтверждено строго (см. «Закрытие»); фикс §010 в lx.14 совпал с исчезновением симптома |
...
**Что менялось lx.12 → lx.14 в download-пути** (ответ на «что могло починить»):
1. **§010 GRO split-brain (`4d4027e4`)** — главный кандидат: «no-detour WG-endpoint killed
   download on android» из-за коалесинга на приёме; ровно тот класс симптома. Бамп submodule.
2. **§011 XHTTP/reality stream-one (`f2654e6a`)** — менял reality-download (`v2rayxhttp/client.go`,
   `conn.go`, новый `reality_detect.go`). Тоже мог повлиять, если зомби шёл через xhttp+reality.
3. §009 AWG masquerade (QUIC/SIP/STUN Initial) — handshake-маскировка, к relay download нерелевантно.

**Честные оговорки (почему «вероятно», а не «точно»):**
- Изначальное «виснет и на VLESS» (раздел «Что НЕ причина») опиралось на корреляцию по времени
  (wlan0 зашифрован, core-log не подтвердил направление) — **не строго подтверждено**. Если оно
  было неточным, §012 — проявление §010 (чисто WG), и lx.14 его закрыл.
...
- Баг был перемежающимся (часть зомби отвисала сама ↓0→↓2820) — отрицательный результат на одной
  сессии не гарантирует, что причина устранена навсегда.
