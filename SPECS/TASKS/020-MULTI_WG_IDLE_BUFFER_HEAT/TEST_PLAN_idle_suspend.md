...
**§NEW RESULT (2026-07-15):** live-прогон выполнен владельцем на CPH2411
(LxBox v2.15.4, ядро v1.14.0-lx.4-rc.2, дефолты клиента: 30s/5m/passive on) —
«всё работает, и прошлое, и текущее». rc.2 промоутнут в стабильный v1.14.0-lx.5.

## §L3 — live-план третьего уровня (lx_idle_teardown), НЕ прогнан

Юнит-покрытие есть (teardown-окно от засыпания, guard-инвариант, цикл
teardown→rebuild на ЖИВОМ gVisor-стеке, идемпотентный Close, шов тика). На
устройстве проверить:

1. **Снос происходит.** Дефолты (30s/5m/teardown=5m): недостижимая нода
   засыпает (`lx idle: suspend`), через ~5 мин сна — `lx idle: teardown <tag>
   slept=5m`. Heap-профиль: netstack ноды (`stack.Stack`) исчез, RSS ниже, чем
   после одного лишь suspend.
2. **Rebuild на дайле.** Дайл через снесённую ноду → `lx idle: rebuild <tag>
   by=dial`, соединение проходит; задержка первого запроса ~0.5–1 с (владелец
   принял). Второй дайл — уже быстрый.
3. **Цикл повторяем.** suspend → teardown → rebuild → suspend → teardown: нет
   утечки горутин/памяти (goroutine profile между циклами ровный).
4. **AWG-guard сильнее.** Guard-опущенный AWG не сносится и не ребилдится
   дайлом (лог guard'а, «not ready» на дайле).
5. **Screen off/on поверх снесённой ноды** — без паник (device=nil), нода
   остаётся снесённой.
