...
| Статус | V (verified) — воспроизведён тестом на живом `Selector`, устранён, регресс закреплён; **полевая проверка пройдена на `v1.14.0-lx.25-rc.7` (2026-08-13): переключение узла срабатывает мгновенно, Stop/Start больше не нужен** |
...
### 3.1 Полевая проверка — пройдена

`v1.14.0-lx.25-rc.7`, 2026-08-13. Переключение узла внутри группы вступает в силу
мгновенно, трафик уходит на новый узел сразу; Stop/Start ядра больше не требуется.
Симптом, с которого начался разбор, устранён.

### 3.2 Открыто

- **UDP-путь** покрыт фиксом по симметрии (`selector.go:182`), но отдельным тестом не закреплён.
...
- **Вложенные группы.** Ветка `ConnectionHandler` не тронута намеренно (см. 2.1): при
  вложенности разрыв обеспечивает внутренняя группа, а не внешний селектор.
