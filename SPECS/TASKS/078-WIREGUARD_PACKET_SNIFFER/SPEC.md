...
| Статус | D (done, device-verified) — код, юниты, сборка `lx-build`, живой прогон через бинарь; полевой прогон на роутере — закрыта решением владельца 2026-09-24: релизы более недели в поле без жалоб |
| Ветка | `lx` |
...
- [x] Полевой прогон на роутере: поток WG с LAN-устройства попадает в
  правило `protocol: wireguard` — закрыт решением владельца 2026-09-24
  (релизы с lx.33 более недели в поле без жалоб).
