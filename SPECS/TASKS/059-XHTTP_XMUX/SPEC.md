...
| Статус | D (done, device-verified) — код + тесты + сборка; полевая проверка на живом Xray-сервере (§7.8) — закрыта решением владельца 2026-09-24: релизы более недели в поле без жалоб |
| Ветка | `lx` |
