# IMPLEMENTATION_REPORT 036 — трасса проб, state-RPC, лог решений

## Файлы

| Зона | Файлы |
|------|-------|
| lx-код | `common/dnstrack/query_trace.go` (заменил `effective_server.go`), `dns/client_log.go`, `dns/transport/group/{group,race}.go`, `daemon/started_service_command_lx.go` (+`GetDNSGroups`, маппинг трассы), `daemon/started_service_command_lx_stub.go`, `experimental/libbox/command_client_command_lx.go` |
| Upstream-шов | `daemon/started_service.proto` — всё внутри существующей `lx:begin lx_command`-зоны; `dns/client.go` — та же 3-строчная зона 035, вызов заменён на `WithQueryTraceIfTracking` |
...
- Write-once `effective` починил атрибуцию вложенных групп (лист, не тег
  внутренней группы) — закреплено юнитом.
...
- В `attempts` — только листовые пробы (участник-группа не пишется:
  её листья рассказывают сами, иначе дубли времени с типом `group`).
...
- Холдер аллоцируется только при активной подписке
  (`WithQueryTraceIfTracking`); без подписчиков — прежний путь.
...
подписка на push-обновления состояния групп (пока pull-RPC — UI опрашивает).
