...
Версия Go-тулчейна, которой собирается AAR, сейчас нигде не записана одним
значением: она размазана по семи workflow-файлам в виде `1.25.x`, `^1.25`,
`^1.25.3`, `^1.25.4` и `1.25.12`. Прочитать её извне нечем, а `go.mod`
для этого не годится — там `1.24.7`, языковой floor, и сборка на нём
воспроизводит [SPEC 044](../044-ANDROID_AAR_GO124_QUIC_DEAD/SPEC.md)
(мёртвые quic-go-аутбаунды на вендорских Android-ядрах).

Задача — завести `go.version` в корне репозитория и сделать его источником
правды для CI.
...
- [ ] `go.version` в корне, содержимое `go1.25.12`
- [ ] Все не-матричные `go-version:` в workflows читают этот файл
- [ ] `go.mod` не изменён
- [ ] CI зелёный, в логе setup-go видна 1.25.12
- [ ] Значение попадает в следующий релизный тег (нужно LxBox для F-Droid)
