...
| Статус | W (wait) — спека 2026-09-24, решения ядра по всем точкам приняты (§4.2); в реализацию; приёмка §5 п. 8 — живой прогон на Windows |
...
**Цена мержа:** ноль апстримных файлов. Правки — пакет `lxd/` (новые файлы с тегом
...
2. Корень, затем `state\`, `logs\` (создаются, если нет) и всё содержимое: владелец →
   Administrators, DACL заменяется целиком на защищённый наследуемый (2.1), у файлов —
   наследуемые ACE родителя.
...
`daemon.json` и state лежат в `<ProgramData>\sing-box-lxd\state\`, лог — в `logs\lxd.log`
...
6. **Секрет после захвата.** Секрет и серверная пара не перегенерируются: лаунчер хранит
   ...
   clients if the host is shared`. Свежая установка без прежнего `daemon.json` — без
   предупреждения.
