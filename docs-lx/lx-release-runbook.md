...
- Наша рабочая ветка — **`lx-1.14`**; интеграция upstream идёт **ручным `git merge upstream/testing`**
  ...
  `wg-1.14-migration` в памяти и [BUILD_CI_CD](../SPECS/FEATURES/001-BUILD_CI_CD/FEATURE.md)).
  `lx-rebase.yml` из [BUILD_CI_CD](../SPECS/FEATURES/001-BUILD_CI_CD/FEATURE.md) описывает старый авто-rebase на **стабильные** теги — он неактуален,
