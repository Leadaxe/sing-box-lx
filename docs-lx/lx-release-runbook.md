...
(`wireguard-go`, `sing-tun`, `gvisor`), поэтому мерж ядра может притащить
зависимость, которая требует API, отсутствующего в базе форка. Ядро при этом
собирается, а AAR — нет, и узнаёшь об этом только от CI.

```bash
for m in wireguard-go sing-tun gvisor; do
  echo "=== $m ==="
  git -C submodules/$m fetch sagernet 2>/dev/null
  git -C submodules/$m log --oneline HEAD..sagernet/main -5   # или sagernet/dev
done
```

⚠️ **Fast-forward форка НЕВОЗМОЖЕН** — в апстримовых ветках ноль наших
lx-коммитов, обновление снесёт AWG2-обфускацию и SPEC 041. Только re-graft или
точечный cherry-pick нужного API.

Реальный случай (SPEC 051, мерж 235 коммитов): апстрим поднял `tailscale`
1.92 → 1.102, тот потребовал `device.PeerLookupFunc`/`NewPeerConfig`/
`SetPeerLookupFunc` из свежего `wireguard-go`, которых нет в базе AWG2-форка.
Цепочка `libbox/native_shell_session.go` → `protocol/tailscale/tailssh` →
`wgengine/wgcfg` → `wireguard-go/device`. Тегами не обходится: `tailssh`
гейтится по `with_gvisor`, который включён всегда. Итог — AAR сломан на уже
влитом мерже.

Если дрейф в сабмодуле есть — сначала закрой его (cherry-pick API / re-graft +
прогон AWG на живом сервере), и только потом мержи ядро.
