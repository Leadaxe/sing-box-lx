...
- **Без TLS клиент остаётся на h2c.** Xray здесь выбрал бы HTTP/1.1; сервер Xray
  принимает обе формы, провод существующих конфигов не меняется.
