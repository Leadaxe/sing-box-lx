...
- ✨ **MASQUE `vhttp: "auto"`** (SPEC 074) — h3 с отвязанным падением на h2 по таймеру
  ...
  40–90 с зависания. **С rc.7 `auto` — дефолт `vhttp`** (v2): MASQUE за TCP-only-хопом
  (`detour`, звено цепочки) спасается сам, а не после ручной правки конфига.
- 🔧 **MASQUE**: смерть туннеля оставляет причину в логе (`masque: tunnel died: …`) —
  слой-виновник цепочки различим по грепу, без пробников (с rc.7).
