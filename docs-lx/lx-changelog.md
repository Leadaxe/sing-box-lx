...
- **NDK запинен на `r28c`** (= `28.2.13676358`) в наших трёх workflow вместо
  плавающего `r28`: CI и локальная сборка AAR идут на одном тулчейне. Тот же
  класс риска, что с версией Go (SPEC 044). Апстримовый `build.yml` не тронут —
