...
**Платформу читать из `os_family`** (`linux`/`darwin`/`windows`), а не из
строки `os`: та человеческая и её формат задаёт дистрибутив. `os_family` нужен,
чтобы отличить «на этой платформе поля не бывает» от «источник молчит».
