...
```jsonc
{
  "outbounds": [
    ...
    // ─────────────────────────────────────────────────────────────────────────
    {
      ...
      "tag": "awg-out",
      "system": false,
      "mtu": 1280,                              // ниже обычного WG (junk s3/s4); ядро ставит 1280 по умолчанию, если заданы s3/s4
      "address": ["10.0.0.2/32"],
...
| `s3` / `s4` | int | по умолчанию `0`. Параметры junk-size AWG 2.0 (компаньоны `s1`/`s2`). Именно их накладные расходы диктуют требование [пониженного MTU](#mtu) |
...
- **`quic`** — полный **QUIC Initial (RFC 9001)**, несущий реалистичный браузероподобный
  ...
  вызов (без фиксированной кросс-юзерной сигнатуры), и `ip=quic` заполняет **и `i1`, и `i2`**
  двумя независимыми Initial'ами, так что поток читается как развивающаяся QUIC-сессия. Это
  device-proven обход DPI (обычный QUIC short header был эмпирически заблокирован).
...
> - Обход DPI держится на **фрагментации CRYPTO-фреймов, а не на TLS/JA3-отпечатке** — мы не
>   имитируем конкретный браузерный fingerprint. `ib` принимается ради совместимости с конфигом
>   WireSock и валидируется, но сейчас не меняет генерируемый ClientHello.
...
**📖 [Подробные примеры →](../SPECS/009-WIRESOCK_MASQUERADE_PROFILES/EXAMPLES.md)** —
...
`s3`/`s4` в AmneziaWG добавляют junk-байты к **каждому transport-сообщению**, поэтому AWG-endpoint требует **более низкий `mtu`, чем обычный WireGuard**. Если обфусцированный пакет превышает path MTU, ОС его отвергает, и туннель завершает handshake, но **не может слать данные**:
...
```
mtu ≤ 1500 − 28 (UDP/IP) − 32 (WireGuard) − max(S3, S4) junk-байт
```

Для `S3 = S4 = 60` это `mtu ≤ 1380`. **Используйте `1280`** (рекомендованный AmneziaWG клиентский MTU) ради запаса на меньших path MTU (PPPoE, вложенные туннели). Это не связано с handshake — слишком высокий `mtu` позволяет handshake пройти, но молча ломает передачу данных.

**Что sing-box-lx делает за вас:** если вы опускаете `mtu` на endpoint'е с `s3`/`s4`, ядро ставит по умолчанию **`1280`** (вместо обычного WireGuard'овского `1408`). Если вы задали `mtu` явно и он слишком велик для junk-расходов, ядро пишет стартовое предупреждение — против консервативного бюджета **1492** байт (PPPoE), `mtu ≤ 1492 − 28 − 32 − max(S3, S4)`, так что оно может пометить значение на несколько байт ниже Ethernet-потолка 1500. Предупреждение рекомендательное; туннель всё равно загружается.
...
Рантайм обеспечивается `Leadaxe/wireguard-go` (sagernet/wireguard-go + обфускация AmneziaWG, подключён через submodule `submodules/wireguard-go`) — см. SPECS/003.
