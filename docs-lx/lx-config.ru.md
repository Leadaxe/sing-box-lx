...
несут тег самой группы (никто не ответил — это и есть состояние). Каждое
событие также несёт трассу проб (SPEC 035): путь групп изнутри наружу,
хронологию проб с исходом (`answered`/`timeout`/`network_error`/`servfail`)
и rtt, признак гонщика. Вызов CommandClient `GetDNSGroups` возвращает живое
состояние групп — режим, победителя/рейтинг/возраст гонки, по участникам
up/down с остатком остывания, счётчик сбоев и последний rtt
(сборки с `with_lx_command`).
