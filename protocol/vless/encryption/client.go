// Package encryption implements the VLESS `encryption` layer
// (mlkem768x25519plus) — a post-quantum handshake that sits inside VLESS,
// beneath the transport and independent of TLS.
//
// lx: ported from the sing-box fork at github.com/starifly/sing-box
// (protocol/vless/encryption), which carries the same GPL-3.0 license and the
// same upstream base as this tree. Client side only: this fork is
// client-focused, so the server half (`decryption`) was deliberately not taken.
// Local helpers that replace its common/xray/{cpuid,crypto} imports live in
// lx_support.go. See SPECS/TASKS/032-VLESS_ENCRYPTION_MLKEM768.
package encryption
...
func (i *ClientInstance) Handshake(conn net.Conn) (*CommonConn, error) {
	...
	c := NewCommonConn(conn, hasAESGCMHardware)
